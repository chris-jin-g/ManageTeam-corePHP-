<?php
	...
	//udpdate todolist
        $select_sql = "SELECT id FROM team_list WHERE team_name='".$team_name."' AND team_escalation='".$team_escalation."' ";
        $result = mysqli_query($conn, $select_sql);
        $idrow = mysqli_fetch_object($result);
        $sql = "insert into todolist values ($idrow->id,'".$team_name."','','','','');";
        //mysqli_($conn, $sql);
	...
	$team_metrics =$_REQUEST['team_metrics'];
        ...
        //mysqli_query($conn, $sql);
 	//update priorities
	$sql = "DELETE FROM todolist WHERE id='".$_REQUEST['team_id']."'";
        //mysqli_query($conn, $sql);
        echo json_encode(array("status" => "success",));
    }    
